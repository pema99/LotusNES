...
namespace LotusNES.Frontend
{
    public partial class NameTableForm : DoubleBufferedForm
    {
        ...
        public NameTableForm(Emulator emu)
            : base(emu)
        {
            InitializeComponent();
        ...
        private void TimerUpdate_Tick(object sender, EventArgs e)
        {
            if (Visible && emu.Running)
            {
                ...
                if (UpdateMirroring)
                {
                    ...
                    ComboMirroring.SelectedIndex = (int)emu.Mapper.VRAMMirroring;
                }
            }
        }
        ...
        private void DrawNameTable(int offsetX, int offsetY, int nameTableNum)
        {
            ...
            for (int i = 0; i < 32; i++)
            {
                ...
                {                  
                    ...
                    int tileNum = emu.PPU.Memory.Read((ushort)(0x2000 + nameTableNum * 0x400 + i + 32 * j));
                    //Fetch palette num from attribute table            Base attribute + offset         + index as 1D, scaled down to 8x8
                    byte paletteNum = emu.PPU.Memory.Read((ushort)(0x23C0 + (nameTableNum * 0x400) + ((i / 4) + 8 * (j / 4))));
                    ...
                    int backgroundPatternTable = ((emu.PPU.GetRegister(0x2000) & 0b00010000) >> 4) * 0x1000;
                    for (int x = 0; x < 8; x++)
                    {
                        for (int y = 0; y < 8; y++)
                        {
                            //Get bitplane bytes
                            byte colLSB = emu.PPU.Memory.Read((ushort)(backgroundPatternTable + tileNum * 16 + y));
                            byte colMSB = emu.PPU.Memory.Read((ushort)(backgroundPatternTable + tileNum * 16 + 8 + y));
                            //Combine them
                            int colNum = ((colLSB >> (7 - x)) & 0b01) | ((colMSB >> (7 - x) << 1) & 0b10);
                            //Combine with palette to get color
                            byte pixel = emu.PPU.Memory.Read((ushort)(0x3F00 + (paletteNum * 4) + colNum));
                            pixel &= 0b111111;
        ...
        private void ComboMirroring_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (emu.Running)
            {
                emu.Mapper.VRAMMirroring = (VRAMMirroringMode)ComboMirroring.SelectedIndex;
            }
        }
